...
export class AuthenticationProvider {
  ...
  constructor(public http: Http, private userInfo: UserInfoProvider, private storage:Storage) {}
    ...
    login(email:string, password:string){
      ...
      return new Promise( (resolve, reject) => {
        this.http.post( this.api_url + '/auth/sign_in', body, options).map(response => {
    ...
    signup(user){
      ...
      body.append('user[confirm_email]', user.confirm_email);
      body.append('user[password]', user.password);
      body.append('user[confirm_password]', user.confirm_password);

      return new Promise( (resolve, reject) => {
        this.http.post( this.api_url +  '/users', body, options).map(response => response.json()) 
        .subscribe(data => {
          resolve(data);
        }, error =>{
          reject(error);
